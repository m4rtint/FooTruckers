...
while($row = mysqli_fetch_array($result))
	...
	if(Auth::check())
   {	
  // Search user_vendors with current vendor's id. If exist, check for favorite. If echo full heart icon, else empty heart icon.
	$sql = "SELECT * FROM ". $username. "_vendors WHERE id =". $row['id'];
	$userlistResult = mysqli_query($con, $sql);
 	$userlistRow = mysqli_fetch_array($userlistResult);
	if ($userlistRow['fav'] == 1)
	...
	else echo "<h1 style='font-size:150%; text-indent:40px'> <input name=favBut".$row['id']. " type=image align=bottom width=16 height=16 src=\"/images/heart_dis.png\" onclick=addFav('". $row['id']. "','". $username."')>  {$row['name']} </h1>";
	}
	else 
	{
		echo "<h1 style='font-size:150%; text-indent:40px'>{$row['name']} </h1>";
  	}
